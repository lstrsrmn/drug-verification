...
class GradNorm:
    """Gradient normalization for two-task training.

    This implementation balances a task loss and a constraint loss by adapting
    task weights online according to the GradNorm objective.
    """
    ...
    def balance(
        ...
        inverse_train_rate = loss_ratio / (
            tf.reduce_mean(loss_ratio) + self.epsilon
        )
        ...
        finite_weights = tf.where(
            tf.math.is_finite(self.weights), self.weights, tf.ones_like(self.weights)
        )
